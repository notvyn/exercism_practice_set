...
namespace estate_executor {
    int assemble_account_number(int secret_modifier) {
        int assembled_account = 0;
        assembled_account += zhang::bank_number_part(secret_modifier);
        assembled_account += khan::bank_number_part(secret_modifier);
        assembled_account += garcia::bank_number_part(secret_modifier);

        return assembled_account;
    }

    int assemble_code() {
        int red_fragment = 0, blue_fragment = 0;

        red_fragment += zhang::red::code_fragment();
        red_fragment += khan::red::code_fragment();
        red_fragment += garcia::red::code_fragment();

        blue_fragment += zhang::blue::code_fragment();
        blue_fragment += khan::blue::code_fragment();
        blue_fragment += garcia::blue::code_fragment();

        int product = red_fragment * blue_fragment;
        return product;
    }
}