...
int monthly_rate(double hourly_rate, double discount) {
    ...
    return ceil(monthly_rate - (monthly_rate * discount / 100));
}
...
int days_in_budget(int budget, double hourly_rate, double discount) {
    ...
    double ratePerDay = daily_rate(hourly_rate);
    ratePerDay -= ratePerDay * discount / 100; 
    return floor(budget / ratePerDay);
}
